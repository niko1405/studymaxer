...
const HomeScreen = () => {
  const { matchesRef, explorerRef, scrollTo } = useHomeScreen();
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (currentHash === '#welcome') {
        ...
        scrollTo('matches', behavior);
      } else if (currentHash === '#explorer') {
        scrollTo('explorer', behavior);
      }
    }, 10);

    return () => clearTimeout(timer);
  }, [currentHash, scrollTo]);
  ...
  return (
    <>
      ...
      <div className="h-screen w-full overflow-y-scroll snap-y snap-mandatory scroll-smooth hide-scrollbar bg-slate-900 text-white">
        <div ref={welcomeRef} className="absolute top-0" />
...
// import { institutions, studyMatches } from '../../../config/mock';
...
//   const { setShowNavigation, setShowNotification } = useStudyMaxer();
...
//       requestAnimationFrame(() => setTimeout(() => setTriggerEntrance(true), 50));
//       setTimeout(() => setAnimFinished(true), 1600);
//     };
//     preloadImages();
//   }, []);

//   // Control Main Nav/Notifs visibility based on fullscreen modes
//   useEffect(() => {
//     if (selectedStudy || selectedInstitution) {
...
//     }
//   }, [selectedStudy, selectedInstitution, setShowNavigation, setShowNotification]);

//   // Touch/Swipe Logic for Top Slider
//   const touchStartX = useRef<number | null>(null);
//   const handleSave = (id: string) => {
//     setSavedItems(prev => prev.includes(id) ? prev.filter(item => item !== id) : [...prev, id]);
//   };
//   const next = () => { if (currentIndex < studyMatches.length - 1) setCurrentIndex(c => c + 1); };
...
//       {/* MAIN SCROLL CONTAINER: Snap Scrolling */}
//       <div className="h-[100dvh] w-full overflow-y-auto snap-y snap-mandatory bg-[#020617] scroll-smooth no-scrollbar">

//         {/* --- SECTION 1: MATCH SLIDER --- */}
//         <section className="h-[100dvh] w-full snap-start shrink-0 flex flex-col relative overflow-hidden">

//           {/* Header */}
//           <div className="h-14 mt-10 shrink-0 flex items-center justify-center px-6 border-b border-gray-800/30">
//             <h1 className="text-xl font-medium tracking-[0.2em] uppercase text-gray-500">Deine Matches</h1>
//           </div>

//           {/* Slider Content */}
//           <div className="flex-1 min-h-0 w-full flex flex-col items-center justify-center py-4 pb-28">
...
//                   <button onClick={prev} disabled={currentIndex === 0} className="pointer-events-auto p-4 rounded-full bg-[#111e33]/90 border border-gray-700/50 text-white hover:bg-gray-700 hover:scale-110 active:scale-95 disabled:opacity-0 disabled:cursor-not-allowed shadow-2xl backdrop-blur-md transition-all">
//                     <ChevronLeft className="w-8 h-8" />
//                   </button>
//                   <button onClick={next} disabled={currentIndex === studyMatches.length - 1} className="pointer-events-auto p-4 rounded-full bg-[#111e33]/90 border border-gray-700/50 text-white hover:bg-gray-700 hover:scale-110 active:scale-95 disabled:opacity-0 disabled:cursor-not-allowed shadow-2xl backdrop-blur-md transition-all">
...
//                   </div>
//                 ) : (
//                   <div className="flex-1 w-full h-full flex items-center touch-pan-y" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
//                     <div className="flex h-full w-full transition-transform duration-500 ease-out will-change-transform" style={{ transform: `translateX(-${currentIndex * 100}%) translateZ(0)` }}>
//                       {studyMatches.map((study, index) => {
//                         let animClass = "opacity-0 translate-y-32 scale-95";
//                         if (triggerEntrance) animClass = "opacity-100 translate-y-0 scale-100 transition-all duration-1000 ease-out";
//                         if (animFinished) animClass = "";
//                         return (
//                           <div key={study.id} className="min-w-full h-full px-4 md:px-8 py-2 flex items-center justify-center backface-hidden">
//                             <div className={`w-full h-full ${animClass}`} style={{ transitionDelay: (!animFinished && triggerEntrance) ? `${index * 80}ms` : '0ms' }}>
//                               <StudyMatchCard
//                                 study={study}
...
//           {/* Pagination Dots (Positioned Higher) */}
//           <div className={`absolute bottom-24 w-full flex items-center justify-center gap-2 z-10 transition-opacity duration-1000 delay-500 ${triggerEntrance ? 'opacity-100' : 'opacity-0'}`}>
//             {studyMatches.map((_, index) => (
//               <button key={index} onClick={() => setCurrentIndex(index)} className={`transition-all duration-300 rounded-full cursor-pointer hover:bg-blue-400 ${index === currentIndex ? 'bg-blue-500 w-8 h-1.5' : 'bg-gray-700 w-1.5 h-1.5'}`} />
//             ))}
//           </div>

//           {/* Hint to scroll down */}
//           <div className="absolute bottom-6 w-full flex justify-center animate-bounce opacity-50">
//              <div className="text-gray-500 text-xs uppercase tracking-widest flex flex-col items-center gap-1">
//                <span>Entdecke mehr</span>
//                <div className="w-0.5 h-6 bg-gradient-to-b from-gray-500 to-transparent"></div>
//              </div>
//           </div>
//         </section>
...
//         <CampusExplorer institutions={institutions} onSelect={setSelectedInstitution} />
